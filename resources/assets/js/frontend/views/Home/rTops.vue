...
<template>
    <v-container>
        ...
        <carousel :perPageCustom="[[100, 1], [600, 2], [1024, 3], [1440, 4], [1920, 5]]" loop :autoplayTimeout="3000" autoplayHoverPause :navigationClickTargetSize="10" navigationEnabled>
            <slide v-for="(top, i) in tops" :key="i">
                <h2 class="subheading white--text">{{ top.position }}º {{ top.title }} - {{ top.artist }}</h2>
                <div class="video--wrapper">
                    <iframe :id="top.id" :src="getVideo(top)" frameborder="0" allowfullscreen :width="width" :height="height"></iframe>
                </div>
            </slide>
        </carousel>
    </v-container>
